...
public class Particle implements Cloneable {
    ...
    public void moveForward(double turn, double forward) {
        ...
        turn = turn + auxiliar.NumberGenerator.randomDoubleGaussian(0, 0, this.turn_noise);
        turn %= 360;
        forward = forward + auxiliar.NumberGenerator.randomDoubleGaussian(0, 0, this.getForward_noise());
        this.x += Math.cos(turn) * forward;
        this.y -= Math.sin(turn) * forward;

        this.orientation = turn;  //retin si orientarea particulei
    }
